...
#    torch.cuda.set_device(3)
 ...
 #   CUDA_VISIBLE_DEVICES = 2
    ...
    check_exist_or_remove("/home/badri/bns332/vae_folding/log/train_loss_log.txt")
    check_exist_or_mkdirs("/home/badri/bns332/vae_folding/log")
    loss_log = open('../log/train_loss_log.txt', 'w+')
    running_loss=0.0
    start = timer()
    print("Entered after timer()")
    for ep in range(0, epoches):
        print("Epoch Number :",str(ep),"Loss:",str(running_loss))
#        print(running_loss)
#        print(running_loss)
        running_loss = 0.0
        for batch_idx, batch in enumerate(dataloader):
            ...
            points_pred,mu,std = model(data)
            KLD = 0.5*(torch.sum( std.pow(2) + mu.pow(2) -  512 - torch.log(std.pow(2)) ) )
            KLD = KLD/(8*512)
            loss = chamfer_distance_loss(data, points_pred) + KLD
#            print("Loss is :",loss)      
        ...
        if save_along_training:
            torch.save(model.state_dict(),os.path.join('/home/badri/bns332/vae_folding/model/', 'ep_%d.pt' % ep))
...
if __name__ == '__main__':
    ...
    TRIAN_PATH = "/home/badri/bns332/vae_folding/train.txt"
   ...
   # MLP_DIMS = (3,1024)
    ...
    check_exist_or_remove('/home/badri/bns332/vae_folding/model')   # clean up old history
    check_exist_or_mkdirs('/home/badri/bns332/vae_folding/model')
    kwargs = {
        'lr': 0.0001,
        # 330
        'epoches': 200
