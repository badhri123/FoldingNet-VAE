#!/usr/bin/env python
'''
train foldingnet on ShapeNet data
...
def train(dataset, model, batch_size, lr, epoches, log_interval, save_along_training):
    ...
    check_exist_or_remove("../log/shape_train_loss_log.txt")
    ...
    for ep in range(0, epoches):
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            if batch_idx % log_interval == log_interval - 1:
                ...
                with open('../log/shape_train_loss_log.txt', 'a+') as f:
                    f.write('[{0:d}, {1:5d}] loss: {2:.6f}\n'.format(ep + 1, batch_idx + 1, running_loss / log_interval))
                running_loss = 0.0
        if save_along_training:
            torch.save(model.state_dict(), os.path.join('../model', 'shape_ep_%d.pth' % ep))
...
#    ROOT = "/media/yuqiong/DATA/city/shapenet/"    # root path
#    TRAIN_PATH = "/media/yuqiong/DATA/city/shapenet_catelog/train.txt"    # root path
    ROOT = "/data/city/shapenet/"    # root path
    TRAIN_PATH = "/data/city/shapenet_catelog/train.txt"    # root path
   ...
   # model = FoldingNetShapes(MLP_DIMS, FC_DIMS, FOLDING1_DIMS, FOLDING2_DIMS)
    model = FoldingNetVanilla(MLP_DIMS, FC_DIMS, FOLDING1_DIMS, FOLDING2_DIMS)
