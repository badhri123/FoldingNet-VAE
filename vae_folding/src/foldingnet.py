...
from reparametrize import sample,codeload
import torch.nn as nn
import torch.nn.functional as Functional
import numpy as np
from torch.autograd import Variable
sys.path.insert(0, '/home/badri/bns332/vae_folding/src')
import pointnet
...
# ./nndistance put side by side of ./src
sys.path.insert(0, '/home/badri/bns332/vae_folding/nndistance')
...
class FoldingNetVanilla(nn.Module):
    # PointNetVanilla or nn.Sequential
    def __init__(self, MLP_dims, FC_dims, grid_dims, Folding1_dims,
        ...
        self.mulayer = nn.Linear(512,512)
        self.stdlayer = nn.Linear(512,512)
        # Decoder
        #   Folding
    ...
    def forward(self, X,codes):
        ...
        f1 = sample(mu,std)
#        f1 = codes[0:64,:]
     #   f = torch.load('codes.pt')
       # print(f1.size(),"Poda punda")
        f = f1.unsqueeze(1)                      # Bx1xK
        ...
        if not X.is_cuda:
            ...
            #codeword = codeword.cuda()
        tmpGrid = tmpGrid.float()
        codeword = codeword.float()    
        ...
        tmpGrid = tmpGrid.float()
        codeword = codeword.float()
        code = codeword.type_as(tmpGrid)
        f = torch.cat((tmpGrid, code), 2)   # BxNx(K+2)
        f = self.Fold1.forward(f)               # BxNx3

        # 2nd folding
        f = torch.cat((f, codeword), 2 )        # BxNx(K+3)
        f = self.Fold2.forward(f)               # BxNx3
        torch.save(f,'vae_insane.pt')
        return f,mu,std
...
class FoldingNetShapes(nn.Module):
    ...
    def forward(self, X):
        ...
        tmpGrid = tmpGrid.float()
        codeword = codeword.float()
        code1= codeword.type_as(tmpGrid)
        f = torch.cat((tmpGrid, code1), 2)  # BxNx(K+9)
        # print(tmpGrid.shape)
