...
class plyDataset(Dataset):
    """
    point cloud dataset but in ply format!!!
    load individual .npy files in a folder
    depend on module open3d to read .ply file contaning point clouds
    """
    def __init__(self, root, roster):
        """
        :param root: root folder for all .npy files
    ...
    def __getitem__(self, idx):
        ply_name = os.path.join(self.root, self.roster[idx])
        pcd = open3d.io.read_point_cloud(ply_name)
        self.x = np.asarray(pcd.points)        
        self.x = torch.FloatTensor(self.x)
